...
class MLP():
    ...
    def full_forward_propagation(self, X):
        ...
        # Input layer.
        memory["A0"] = A_curr
        for layer_number, layer in enumerate(self.nn_architecture, 1):
            W = self.params_values["W" + str(layer_number)]
            b = self.params_values["b" + str(layer_number)]
            # Determine the activation function.
            func = layer["activation"]
            A_curr, Z_curr = self.single_layer_forward_propagation(
                A_curr, W, b, func)
            memory["A" + str(layer_number)] = A_curr
            memory["Z" + str(layer_number)] = Z_curr
    ...
    def get_cost_value(self, Y_hat, Y):
        ...
        cost = (-1/m)*(self.matmul(Y, np.log(Y_hat.T)) +
                       self.matmul((1-Y), np.log(1-Y_hat.T)))
    ...
    def update(self, grads_values, learning_rate):
        ...
        for layer_number, layer in enumerate(self.nn_architecture):
            layer_number += 1
            self.params_values["W" + str(layer_number)] -= learning_rate * \
                grads_values["dW" + str(layer_number)]
            self.params_values["b" + str(layer_number)] -= learning_rate * \
                grads_values["db" + str(layer_number)]
    ...
    def train(self, X, Y, epochs, learning_rate, verbose=False):
        ...
        for i in range(epochs):
            ...
            gradients = self.full_backward_propagation(y_hat, Y, memory)
